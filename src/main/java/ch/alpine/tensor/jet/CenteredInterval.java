...
public class CenteredInterval extends AbstractScalar implements //
  ...
  protected Scalar plus(Scalar scalar) {
    if (scalar instanceof CenteredInterval) {
      CenteredInterval centeredInterval = (CenteredInterval) scalar;
      return new CenteredInterval( //
  ...
  public Scalar multiply(Scalar scalar) {
    if (scalar instanceof CenteredInterval) {
      CenteredInterval centeredInterval = (CenteredInterval) scalar;
      return new CenteredInterval(Outer.of(Scalar::multiply, flat(), centeredInterval.flat()).flatten(1) //
          .map(Scalar.class::cast) //
          .collect(MinMax.toClip()));
    }
    return of(flat().multiply(scalar).stream().map(Scalar.class::cast).collect(MinMax.toClip()));
  }
  ...
  public boolean equals(Object object) {
    return object instanceof CenteredInterval //
        && clip.equals(((CenteredInterval) object).clip);
  }
