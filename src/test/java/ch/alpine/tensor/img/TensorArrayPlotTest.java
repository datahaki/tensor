...
public class TensorArrayPlotTest extends TestCase {
  public void testSmall() {
    Tensor matrix = Tensors.fromString("{{0, 0.1}}");
    Tensor image = TensorArrayPlot.of(matrix, ColorDataGradients.CLASSIC);
    assertEquals(Dimensions.of(image), Arrays.asList(1, 2, 4));
  }

  public void testHue() {
    Tensor matrix = Tensors.fromString("{{0, 0.1}, {1, 2}, {1.2, 0.2}}");
    Tensor image = TensorArrayPlot.of(matrix, HueColorData.DEFAULT);
